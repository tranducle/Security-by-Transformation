...
def find_api_key() -> Optional[str]:
    ...
    2. PROJECT/.agent/skills/ai-multimodal/.env (skill-specific)
    3. PROJECT/.agent/skills/.env (shared skills)
    4. PROJECT/.claude/.env (project global)
    5. ~/.agent/skills/ai-multimodal/.env (user skill-specific)
    6. ~/.agent/skills/.env (user shared)
...
def batch_process(
    ...
    if not api_key:
        ...
        print("4. Create .env: cd ~/.agent/skills/ai-multimodal && cp .env.example .env")
