...
Examples:
  ...
  python -m src.langgraph.cli --test
    ...
    parser.add_argument(
        "--json", "-j",
        action="store_true",
        help="Output structured JSON (for host AI consumption)",
    )
    parser.add_argument(
        "--health",
        action="store_true",
    ...
    if args.request:
        if args.quiet:
            ...
            # Structured JSON output for host AI (Antigravity)
            result = run_graph(args.request)
            structured = result.get("structured_output", {})
            # Truncate system_prompt to save tokens
            if "agent_context" in structured:
                prompt = structured["agent_context"].get("system_prompt", "")
                if len(prompt) > 2000:
                    structured["agent_context"]["system_prompt"] = prompt[:2000] + "...[truncated]"
