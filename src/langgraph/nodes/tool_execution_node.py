...
from src.langgraph.tools_integration import get_tool_category, get_tool_callable
...
# ============================================================================
# TOOL FALLBACKS — when a tool fails, try these alternatives
# ============================================================================

TOOL_FALLBACKS: Dict[str, List[str]] = {
    # Literature search fallbacks
    "search_scopus_sync": ["search_openalex_sync", "search_semantic_scholar_sync"],
    "search_semantic_scholar_sync": ["search_openalex_sync", "search_google_scholar"],
    "search_openalex_sync": ["search_semantic_scholar_sync", "search_google_scholar"],
    "search_google_scholar": ["search_semantic_scholar_sync", "search_openalex_sync"],
    # Math verification fallbacks (Wolfram → SymPy)
    "wolfram_query": ["verify_equation_sympy"],
    "verify_equation": ["verify_equation_sympy"],
    "simplify_expression": ["simplify_expression_sympy"],
    "compute_derivative": ["compute_derivative_sympy"],
    "compute_integral": ["compute_integral_sympy"],
}
...
def tool_execution_node(state: ResearchState) -> ResearchState:
    ...
    if to_execute:
        ...
        if on_failure == "stop":
            ...
            # "skip" or "retry" — group by category for smarter parallelism
            executed_results = _execute_with_grouping(to_execute, exec_cfg)

    # ---- Dynamic Replanning: try fallback tools for failures ----
    failed_tools = [
        r for r in executed_results if r.get("status") not in ("success",)
    ]
    if failed_tools and on_failure != "stop":
        replan_results, replan_entries = _replan_failed_tools(
            failed_tools, user_message, exec_cfg
        )
        executed_results.extend(replan_results)
        state["replan_log"].extend(replan_entries)
        state["replan_count"] = len(replan_entries)
        if replan_entries:
            state["execution_log"].append(
                f"[REPLAN] Tried {len(replan_entries)} fallback tools"
            )
...
# ============================================================================
# TOOL GROUPING — group by category for smarter parallel execution
# ============================================================================


def _execute_with_grouping(
    tools: List[Dict[str, Any]], exec_cfg: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """Execute tools grouped by category.

    Tools in the same category (e.g., 4 literature search APIs) run
    concurrently. Different category groups run sequentially so that
    dependent categories (e.g., writing after literature) have results.

    Falls back to flat parallel execution if grouping yields one group.
    """
    # Group tools by their category
    groups: Dict[str, List[Dict[str, Any]]] = {}
    for tool in tools:
        category = get_tool_category(tool.get("tool_name", ""))
        groups.setdefault(category, []).append(tool)

    if len(groups) <= 1:
        # Only one category — just run all in parallel
        return execute_tools_parallel(tools, exec_cfg)

    # Execute groups sequentially, tools within each group in parallel
    all_results: List[Dict[str, Any]] = []
    for category, group_tools in groups.items():
        logger.info(
            "Executing tool group '%s': %d tools in parallel",
            category,
            len(group_tools),
        )
        group_results = execute_tools_parallel(group_tools, exec_cfg)
...
# ============================================================================
# DYNAMIC REPLANNING — fallback tools when primary tools fail
# ============================================================================


def _replan_failed_tools(
    failed_results: List[Dict[str, Any]],
    user_message: str,
    exec_cfg: Dict[str, Any],
) -> tuple:
    """Try fallback tools for each failed tool.

    Args:
        failed_results: List of ToolResult dicts that failed
        user_message: Original user message for auto-fill
        exec_cfg: Execution config

    Returns:
        Tuple of (fallback_results, replan_log_entries)
    """
    fallback_results: List[Dict[str, Any]] = []
    replan_log: List[str] = []
    already_tried = {r.get("tool_name") for r in failed_results}

    for failed in failed_results:
        tool_name = failed.get("tool_name", "")
        original_kwargs = failed.get("args", {})
        fallbacks = TOOL_FALLBACKS.get(tool_name, [])

        for fb_name in fallbacks:
            if fb_name in already_tried:
                continue

            fb_callable = get_tool_callable(fb_name)
            if fb_callable is None:
                continue

            # Build kwargs — try original kwargs, fallback to user_message query
            fb_kwargs = dict(original_kwargs)
            if "query" not in fb_kwargs:
                fb_kwargs["query"] = user_message

            log_entry = f"{tool_name} failed → trying {fb_name}"
            replan_log.append(log_entry)
            logger.info("[REPLAN] %s", log_entry)

            timeout = exec_cfg.get("tool_timeout", 30)
            retries = exec_cfg.get("tool_retries", 2)
            result = execute_tool_sync(fb_name, fb_callable, fb_kwargs, timeout, retries)
            already_tried.add(fb_name)

            fallback_results.append(result)

            if result.get("status") == "success":
                # First successful fallback is enough
                break

    return fallback_results, replan_log
