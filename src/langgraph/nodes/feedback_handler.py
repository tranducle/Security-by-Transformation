...
def detect_feedback(message: str) -> Tuple[bool, List[str]]:
    ...
    To prevent false positives from system-generated content that might
    contain words like "fix", "error", or "wrong", we require at least
    TWO pattern matches before treating as feedback.
    ...
    # Require at least 2 matches to prevent false positives from
    # system-generated content containing words like "fix", "error"
    # This prevents oscillation where the system's own output
    # triggers feedback mode.
    MIN_MATCHES_FOR_FEEDBACK = 2
    return len(matched) >= MIN_MATCHES_FOR_FEEDBACK, matched
