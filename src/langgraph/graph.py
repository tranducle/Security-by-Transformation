...
from .nodes.validate_output import validate_output_node, should_retry
from .nodes.checkpoint_saver import checkpoint_saver_node
from .nodes.context_compressor import context_compressor_node
from .nodes.sop_orchestrator import sop_orchestrator_node
...
from .nodes.human_approval import human_approval_node, should_proceed_after_approval
...
def agent_loading_node(state: ResearchState) -> ResearchState:
    ...
    # Handle SOP pipeline routing — for SOP targets, load the FIRST agent's
    # config for context. The actual multi-agent execution happens in
    # sop_orchestrator_node (for the LangGraph path) or inline (for fallback).
...
def _route_after_routing(state: ResearchState) -> str:
    """Decide whether to use single-agent or SOP pipeline path."""
    target = state.get("target_agent") or ""
    if target.startswith("SOP:"):
        return "sop"
    return "single"
...
def _build_graph():
    ...
    # ---- NEW nodes ----
    workflow.add_node("context_compress", context_compressor_node)
    workflow.add_node("validate_output", validate_output_node)
    workflow.add_node("sop_orchestrator", sop_orchestrator_node)
    workflow.add_node("checkpoint", checkpoint_saver_node)
    workflow.add_node("human_approval", human_approval_node)  # Phase 5

    # ---- Entry ----
    workflow.set_entry_point("feedback_detector")

    # ---- Pre-routing chain (unchanged) ----
    ...
    # ---- CONDITIONAL: single agent vs SOP pipeline ----
    workflow.add_conditional_edges(
        "route",
        _route_after_routing,
        {"single": "load_agent", "sop": "sop_orchestrator"},
    )

    # ---- Single agent path ----
    workflow.add_edge("load_agent", "load_tools")
    workflow.add_edge("load_tools", "load_memory")
    workflow.add_edge("load_memory", "context_compress")
    workflow.add_edge("context_compress", "human_approval")   # Phase 5

    # ---- Human approval gate ----
    workflow.add_conditional_edges(
        "human_approval",
        should_proceed_after_approval,
        {"proceed": "tool_execution", "reject": "output_builder"},
    )

    workflow.add_edge("tool_execution", "validate_output")

    # ---- Quality gate conditional ----
    workflow.add_conditional_edges(
        "validate_output",
        should_retry,
        {"pass": "output_builder", "retry": "tool_execution"},
    )

    # ---- SOP path converges at validate_output ----
    workflow.add_edge("sop_orchestrator", "validate_output")

    # ---- Post-output ----
    workflow.add_edge("output_builder", "checkpoint")         # NEW
    workflow.add_edge("checkpoint", END)

    logger.info(
        "Compiled LangGraph research graph with conditional routing, "
        "quality gate, SOP orchestrator, human approval, and checkpoint saver (singleton)"
    )
...
def run_without_langgraph(user_message: str) -> Dict[str, Any]:
    ...
    Mirrors the LangGraph graph structure including new nodes.
    """
    ...
    # Conditional: SOP vs single agent
    if (state.get("target_agent") or "").startswith("SOP:"):
        state = sop_orchestrator_node(state)
    else:
        ...
        if state.get("approval_status") != "approved":
            # Skip tool execution, go to output
            state = output_builder_node(state)
            state = checkpoint_saver_node(state)
            return state

        state = tool_execution_node(state)

    # Validate output with retry loop
    for _attempt in range(3):
        state = validate_output_node(state)      # NEW
        validation = state.get("validation_result", {})
        if validation.get("passed", True) or state.get("retry_count", 0) > 2:
            break
        state = tool_execution_node(state)

    state = output_builder_node(state)
    state = checkpoint_saver_node(state)          # NEW
