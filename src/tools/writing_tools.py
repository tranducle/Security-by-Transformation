...
# Allowed directories for file writing (prevents path traversal)
ALLOWED_WRITE_DIRS = [
    Path.cwd() / "outputs",
    Path.cwd() / "references",
    Path.cwd() / "papers",
...
def _validate_write_path(filepath: str) -> Path:
    """
    Validate that the filepath is within allowed directories.
    ...
    Raises:
        ValueError: If path is outside allowed directories (path traversal)
    """
    ...
    if not is_allowed:
        # Also allow if path is within current working directory
        cwd = Path.cwd().resolve()
        if not str(path).startswith(str(cwd)):
            raise ValueError(
                f"Path traversal detected: '{filepath}' is outside allowed directories. "
                f"Allowed directories: {[str(d) for d in ALLOWED_WRITE_DIRS if d.exists()]}"
            )
...
def write_markdown_section(
    ...
    Raises:
        ValueError: If filepath is outside allowed directories (path traversal)
    """
    # Validate path to prevent path traversal attacks
    path = _validate_write_path(filepath)
